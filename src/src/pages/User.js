...
export default function AddUser() {
  ...
  const { user_id, firstname, lastname, email_address, gender, user_type, phones } = user;
  ...
  const handlePhoneChange = (e, index) => {
    ...
    setUser({ ...user, phones: newPhones });
  }


  const addPhoneInput = () => {
    setUser({ ...user, phones: [...phones, ''] }); // Add a new empty phone number input
  }

  const onSubmit = async (e) => {
    e.preventDefault();
    
    await axios.post("http://localhost:8000/create-user", user); // Update the endpoint URL if necessary
    navigate("/home");
  }
